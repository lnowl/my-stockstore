...
export default function StockStoreApp() {
  ...
  return (
    <div className="min-h-screen text-gray-200 font-sans" style={{ fontFamily: 'Sarabun, sans-serif', backgroundColor: '#212121' }}>
      <div className="max-w-7xl mx-auto p-4">
        ...
        {/* Dashboard */}
        {showDashboard && (
          <div className="bg-gray-900 p-4 rounded shadow mb-6" style={{ backgroundColor: '#303030' }}>
            <h2 className="text-xl font-bold mb-3 text-white">Dashboard สรุปสต๊อก</h2>
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div className="p-3 rounded flex items-center gap-2" style={{ backgroundColor: '#565656' }}>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="ค้นหาสินค้า" className="p-2 rounded  text-white flex-1" style={{ backgroundColor: '#303030' }}/>
          <select value={category} onChange={e => setCategory(e.target.value)} className="p-2 rounded bg-gray-800 text-white"style={{ backgroundColor: '#303030' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {filtered.map(p => (
            <div key={p.id} className="p-4 rounded shadow hover:shadow-lg transition cursor-pointer" style={{ backgroundColor: '#303030' }}>
        ...
        {showAdmin && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="p-6 rounded shadow-lg w-full max-w-md" style={{ backgroundColor: '#212121' }}>
              ...
              <form onSubmit={handleAddOrUpdateProduct} className="flex flex-col gap-3">
                <input required placeholder="ชื่อสินค้า" value={newProduct.name} onChange={e => setNewProduct({...newProduct, name: e.target.value})} className="p-2 rounded bg-gray-800 text-white"style={{ backgroundColor: '#303030' }} />
                <input required placeholder="หมวดหมู่" value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})} className="p-2 rounded bg-gray-800 text-white" style={{ backgroundColor: '#303030' }}/>
                <input required placeholder="SKU" value={newProduct.sku} onChange={e => setNewProduct({...newProduct, sku: e.target.value})} className="p-2 rounded bg-gray-800 text-white"style={{ backgroundColor: '#303030' }} />
                <input required placeholder="ราคา" type="number" value={newProduct.price} onChange={e => setNewProduct({...newProduct, price: e.target.value})} className="p-2 rounded bg-gray-800 text-white" style={{ backgroundColor: '#303030' }}/>
                <input required placeholder="จำนวนคงเหลือ" type="number" value={newProduct.stock} onChange={e => setNewProduct({...newProduct, stock: e.target.value})} className="p-2 rounded bg-gray-800 text-white" style={{ backgroundColor: '#303030' }}/>
                <input required placeholder="หน่วย" value={newProduct.unit} onChange={e => setNewProduct({...newProduct, unit: e.target.value})} className="p-2 rounded bg-gray-800 text-white" style={{ backgroundColor: '#303030' }}/>
                <input required placeholder="ผู้จัดหา" value={newProduct.supplier} onChange={e => setNewProduct({...newProduct, supplier: e.target.value})} className="p-2 rounded bg-gray-800 text-white" style={{ backgroundColor: '#303030' }}/>
